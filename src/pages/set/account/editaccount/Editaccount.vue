<template>
    <div class="content">
        ...
        <div class="addpage-box">
            ...
            <ul class="add-ul">
                <li>
                    <span class="t-name">角色名称</span><input class="t-ipt" style="background-color:#eee;" v-model="model1" type="text" placeholder="请输入用户名" disabled />
                    <!-- <Select v-model="model1" style="width:200px">
                        <Option v-for="item in jueseList" :value="item.ag_dj" :key="item.ag_dj">{{ item.ag_name }}</Option>
                    </Select> -->
                </li>
                <li>
                    <span class="t-name">用户名</span><input class="t-ipt" v-model="name" type="text" placeholder="请输入用户名" />
                </li>
                <li>
                    <span class="t-name">用户密码</span><input class="t-ipt" v-model="pwd" type="password" placeholder="请输入密码" />
                </li>
                <li>
                    <span class="t-name">手机号码</span><input class="t-ipt" v-model="mobile" type="text" placeholder="请输入手机号码" />
                </li>
                <li>
                    <span class="t-name"></span><button class="t-btn" @click="addrole">保存</button>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    created () {
        ...
        this.api.apiPost('/admin/account', 'post', data, params, (status, code, res) => {
            if (status) {
                var obj = new Object();
                obj = res.data;
                obj.admid = this.sid;
                this.editdata = obj;
                //console.log("数据数据："+JSON.stringify(this.editdata));
                this.id = this.editdata.admid;
                this.model1 = this.editdata.group.ag_name;
                this.name = this.editdata.admin.admin_username;
                this.mobile = this.editdata.admin.admin_mobile;
            } else {
    ...
    methods: {
        ...
        jueseFun () {
            ...
            this.api.apiPost('/admin/account', 'get', '', params, (status, code, res) => {
                if (status) {
                    //this.jueseList = res.data;
                    this.model1 = res.data[0].ag_name;
                } else {
