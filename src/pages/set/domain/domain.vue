<template>
  <div class="content">
    <div class="addpage-box">
      <div class="add-title">分享域名更改</div>
      <ul class="add-ul">
        <li>
          <span class="t-name">分享域名</span>
          <el-input v-model="gongData" placeholder="请输入新的域名"></el-input>
        </li>
        <li>
...
export default {
  ...
  methods: {
    ...
    getewm() {
      ...
      this.api.apiPost("/wechat/sharedomain","get","",params,(status, code, res) => {
            if (status) {
     ...
     updataEwm() {
      ...
      var data = qs.stringify({
        ...
        Url:this.gongData
         ...
         this.api.apiPost("/wechat/sharedomain","post", data, params,(status, code, res) => {
          if (status) {
            this.$Message.success("更新域名成功！");
          } else {
            this.$Message.error("" + res.msg);
          }
        }
