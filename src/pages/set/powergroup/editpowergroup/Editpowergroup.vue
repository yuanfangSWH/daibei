<template>
    <div class="content">
        <div class="addbtn"><router-link to="/set/powergroup"><button>返回</button></router-link></div>
        <div class="addpage-box">
            <div class="add-title">添加权限分组</div>
            <ul class="add-ul">
                <li>
                    <span class="t-name">分组名称</span><input class="t-ipt" v-model="name" type="text" placeholder="请输入权限描述" />
                </li>
                <li>
                    <span class="t-name"></span><button class="t-btn" @click="addgroup">保存</button>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    created () {
        ...
        this.api.apiPost('/admin/powergroup', 'post', data, params, (status, code, res) => {
            if (status) {
                this.databox = res.data;
                this.name = this.databox.name;
            } else {
