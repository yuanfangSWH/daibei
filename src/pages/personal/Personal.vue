<template>
    <div class="content">
        <div class="addbtn"><router-link to="/personal/editinfo"><button>编辑信息</button></router-link></div>
        <div class="addpage-box">
            <div class="add-title">个人信息</div>
            <ul class="add-ul">
                <li>
                    <span class="t-name">用户id：</span><div class="per-div">{{databox.admin_id}}</div>
                    <!-- <input class="t-ipt" v-model="databox.admin_id" type="text" placeholder="请输入权限描述" disabled /> -->
                </li>
                <li>
                    <span class="t-name">用户名：</span><div class="per-div">{{databox.admin_username}}</div>
                </li>
                <li>
                    <span class="t-name">手机号码：</span><div class="per-div">{{databox.admin_mobile}}</div>
                </li>
                <li>
                    <span class="t-name">等级名称：</span><div class="per-div">{{databox.admin_djname}}</div>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    methods: {
        dataFun () {
            ...
            this.api.apiPost('/admin/personal', 'post', '', params, (status, code, res) => {
                if (status) {
...
.add-ul li{ width: 100%; float: left;}
.t-name{ width: 70px; margin-right: 20px; float: left; text-align: right; font-size:14px;font-family:PingFang-SC-Medium;font-weight:500;color:rgba(0,0,0,1); float: left; height: 40px; display: block; line-height: 40px;}
...
.per-div{ float: left; height: 40px; line-height: 40px;}
</style>