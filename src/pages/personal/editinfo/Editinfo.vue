<template>
    <div class="content">
        <div class="addbtn"><router-link to="/personal"><button>返回</button></router-link></div>
        <div class="addpage-box">
            <div class="add-title">编辑个人信息</div>
            <ul class="add-ul">
                <li>
                    <span class="t-name">用户名</span><input class="t-ipt" v-model="name" type="text" placeholder="请输入用户名" />
                </li>
                <li>
                    <span class="t-name">密码</span><input class="t-ipt" v-model="pwd" type="text" placeholder="请输入密码" />
                </li>
                <li>
...
export default {
    ...
    methods: {
        editFun () {
            ...
            this.api.apiPost('/admin/personal', 'post', data, params, (status, code, res) => {
                if (status) {
        ...
        infoData (){
            ...
            this.api.apiPost('/admin/personal', 'post', '', params, (status, code, res) => {
                if (status) {
                   this.name = res.data.admin_username;
                   this.mobile = res.data.admin_mobile;
                } else {
