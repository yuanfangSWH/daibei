<template>
    <div class="content">
        ...
        <div class="addpage-box">
            <div class="add-title">编辑问题</div>
            <ul class="add-ul">
                <li>
                    <span class="t-name">标题</span><input class="t-ipt" v-model="formItem.informTitle" type="text" placeholder="请输入标题" />
                </li>
                <li>
                    <span class="t-name">问题内容</span><Input v-model="formItem.informtext" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入问题内容" />
                </li>
                <li>
    ...
    created () {
        ...
        this.api.apiPost('/system/quean', 'post', data, params, (status, code, res) => {
            if (code == 200) {
                ...
                //console.log("接受的数据："+ JSON.stringify(this.informData));
                this.formItem.id = this.informData.id;
                this.formItem.informTitle = this.informData.notice_name;
                this.formItem.informtext = this.informData.notice_content;
