...
<template>
    <div class="Upload">
        <Upload v-if="UploadStyle == 'no'" multiple type="drag" :accept="Accept" :format="Format" :max-size="FileMaxSize" :before-upload="HandleUpload" :action="UploadUrl" :data="UploadParams">
            <div style="text-align:center;">
                <!-- <Icon type="ios-monitor-outline" style="font-size: 32px;color: #c5c5c5;"></Icon> -->
                <img src="/static/images/pc_addfile_icn_n.png" style="margin-bottom: 6px;">
                <p style="color: #999;">从本地添加</p>
            </div>
        </Upload>
        <Button v-if="UploadStyle == 'has'" style="padding: 0px 0px;" type="ghost" @click="AgainUpload">
            <Upload multiple type="drag" :accept="Accept" :format="Format" :max-size="FileMaxSize" :before-upload="HandleUpload" :action="UploadUrl" :data="UploadParams">
                本地添加
            </Upload>
        </Button>
        <Modal :title="ModalTitle" v-model="UploadModal" :styles="{top:'348px'}" width="420">
            <div style="text-align:center">
                <div style="margin: 20px 10px;">
                    ...
                    <div v-else-if="UploadState == 'on-progress'">
                        <div class="demo-upload-list">
                            <Progress :percent="PercentNum" :status="ProgressStatus" hide-info></Progress>
                        </div>
                    ...
                    <div v-else-if="UploadState == 'on-error'" style="height: 30px;">
                        <div class="demo-upload-list" style="float: left;margin-left: 100px;">
                            <img :src="ErrorImgUrl" style="width: 24px;height: 24px;">
                        </div>
                        <div class="demo-upload-list" style="width: 200px;float: left;margin-left: 20px;text-align: left;">
                            <span style="font-size: 16px;font-weight:bold;">文件上传失败</span>
                            <br/>
            ...
            <div slot="footer">
                <div v-if="UploadState == 'before-upload'">
                    <Checkbox class="center-checkbox" v-model="SynchronousCloud">同步至云平台“个人中心资源”</Checkbox>
                    <Button type="ghost" @click="UploadModal = false" style="border: solid 1px #dcdcdc;color: #888;">取消</Button>
                    <Button type="primary" @click="Upload" :loading="loadingStatus">上传</Button>
                </div>
                <div v-else-if="UploadState == 'on-progress'" style="text-align: center;">
                ...
                <div v-else-if="UploadState == 'on-success'" style="text-align: center;">
                    ...
                    <Button v-if="FilesNum < 5" type="ghost" @click="AgainUpload" style="padding: 0px 0px;">
                        <Upload multiple type="drag" :accept="Accept" :format="Format" :max-size="FileMaxSize" :before-upload="HandleUpload" :action="UploadUrl" :data="UploadParams">
                            继续上传
                        </Upload>
                    </Button>
...
export default {
  ...
  data() {
    return {
      ...
      Accept: ".pdf,.pptx,.docx,.xlsx,.png,.jpg,.jpeg,.gif,.mp4,.mpeg,.mpg,.avi,.mov,.wmv,.mkv,.flv,.mp3,.wave,.wma",//上传文件格式限制
      Format: ['.pdf','.pptx','.docx','.xlsx','.png','.jpg','.jpeg','.gif','.mp4','.mpeg','.mpg','.avi','.mov','.wmv','.mkv','.flv','.mp3','.wave','.wma'],//上传文件格式限制
