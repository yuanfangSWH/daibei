<template>
    <div class="content">
        <div class="addbtn"><router-link to="/bbs/Bbstag"><button>返回</button></router-link></div>
        <div class="addpage-box">
            <div class="add-title">社区板块编辑</div>
            <ul class="add-ul">
                <li>
                ...
                <li>
                    <span class="t-name"></span><button class="t-btn" @click="editNewstag">保存</button>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    created () {
        ...
        this.api.apiPost('/bbs/tags', 'post', data, params, (status, code, res) => {
            if (status) {
    ...
    methods: {
        ...
        tagInfo () {
            ...
            //console.log("添加：" + JSON.stringify(this.taginfo));
        },
    },
    mounted () {
        this.tagInfo();
