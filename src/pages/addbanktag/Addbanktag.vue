<template>
    <div class="content">
        <div class="addbtn"><router-link to="/Banktag"><button>返回</button></router-link></div>
        <div class="addpage-box">
            <div class="add-title">拉新产品标签添加</div>
            <ul class="add-ul">
                <li>
                    <span class="t-name">标签名</span><input class="t-ipt" v-model="tagname" type="text" placeholder="请输入拉新产品标签名称" />
                </li>
                <li>
                    <span class="t-name">排序</span><input class="t-ipt" v-model="tagrank" type="text" placeholder="请输入排序" /><div class="demo-nav">必填</div>
                </li>
                <li>
                    <span class="t-name"></span><button class="t-btn" @click="addBanktag">保存</button>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    methods: {
        addBanktag () {
            ...
            this.api.apiPost('/bank/tags', 'post', data, params, (status, code, res) => {
                if (status) {
